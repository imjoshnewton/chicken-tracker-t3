...
const Modal = ({ flockId }: { flockId: string | undefined }) => {
    ...
    async function createNewLog(flockId: string, date: Date, count: number, notes?: string): Promise<void> {
    ...
    return (
        <>
            <button
                className="px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 btn"
                type="button"
                            ...
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                ...
                                <div className="relative flex-auto">
                                    <form className="px-8 pt-6 pb-8 w-full">
                                        ...
                                        <input className="appearance-none border rounded w-full py-2 px-1 text-black" required onChange={e => {
                                        ...
                                        <input className="appearance-none border rounded w-full py-2 px-1 text-black" required value={count} onChange={e => setCount(Number(e.target.value))}  placeholder="0" type='number' />
                                        <label className="block text-black text-sm font-bold mb-1">
                                            Notes
                                        </label>
                                        <textarea className="appearance-none border rounded w-full py-2 px-1 text-black" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes..." />
                                    </form>
                                </div>
